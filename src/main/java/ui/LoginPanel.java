...
public class LoginPanel extends JPanel {
    ...
    private void handleLogin() {
        ...
        if (email.isEmpty() || password.isEmpty()) {
            showAlert("Vui lòng nhập đầy đủ email và password (user1/password1 hoặc user2/password2).", 
        ...
        try {
            logger.info(() -> "Login demo thành công: " + email);
            callback.onLoginSuccess();
        } catch (Exception ex) {
            logger.severe(() -> "Lỗi chuyển màn hình: " + ex.getMessage());
            showAlert("Lỗi chuyển màn hình: " + ex.getMessage(), JOptionPane.ERROR_MESSAGE);
